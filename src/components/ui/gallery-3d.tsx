...
function GalleryScene({
	...
	useFrame((state, delta) => {
		// Drive gallery movement from scroll progress delta
		const progressDelta = scrollProgress - prevProgressRef.current;
		prevProgressRef.current = scrollProgress;

		// Convert scroll delta to velocity (scale up for visible movement)
		const scrollDrive = progressDelta * 80 * speed;
		setScrollVelocity((prev) => prev + scrollDrive);
